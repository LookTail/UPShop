...
* 建立一个定时任务，将回调回来的支付成功却在数据库中无法查到对应信息，或查到订单但因为某些原因修改数据库订单状态失败的订单加入该定时任务的缓存中存储。定时遍历该任务中的订单信息进行数据库查询，若查到则进行支付成功的信息更新，若多次查询无果或查询成功却更新状态失败的订单信息进行收集，考虑进行人工审核操作。

* 此类订单在定时任务执行的过程中，用户依然可以查询到正确的订单信息。在定时任务中使用缓存存储支付成功但存在持久化或更新操作问题的订单，用户查询订单时可将数据库中的查询结果与该缓存中的订单信息合并后进行返回。
